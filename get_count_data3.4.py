...
PARSER.add_argument("-n", "--nthreads", required=False,
                    help="number of threads", default=cpu_count())
...
def process_reads(chrom_bam):
    ...
    bam_parse = pysam.AlignmentFile(bam, "rb")
    ###
    ...
    for strain in THE_STUFF:
        outbam = pysam.AlignmentFile(f"{ARGDICT['outdir']}/{bam_prep}_{strain}_{chrom}_temp.bam", "wb", template=bam_parse)
    ...
    if ARGDICT["tosser"]:
        outbam = pysam.AlignmentFile(f"{ARGDICT['outdir']}/{bam_prep}_{'Tosser'}_{chrom}_temp.bam", "wb", template=bam_parse)
        read_stuff = bam_parse.fetch(contig=chrom, start=0)
...
def process_bam(bam):
    '''processed the actual bam file'''
    print(f"processing {bam}")
    bam_prep = bam.split("/")[-1].split(".")[0]
    chrom_bam = []
    for chrom in CHROMS:
        chrom_bam.append({"chrom": chrom, "bam": bam})
    p = Pool(ARGDICT["nthreads"])
    p.map(process_reads, chrom_bam)
    if ARGDICT["tosser"]:
        all_mergers = THE_STUFF | set (["Tosser"])
    else:
        all_mergers = THE_STUFF
    for ds in all_mergers:
        temp_bams = glob.glob(f"{ARGDICT['outdir']}/{bam_prep}_{ds}_*_temp.bam")
        subprocess.call(f"samtools merge {ARGDICT['outdir']}/{bam_prep}_{ds}_merged.bam {' '.join(temp_bams)}", shell=True)
        subprocess.call(f"samtools sort -@ {ARGDICT['nthreads']} -T /tmp/{ds}.sorted -o {ARGDICT['outdir']}/{bam_prep}_{ds}.bam {ARGDICT['outdir']}/{bam_prep}_{ds}_merged.bam && samtools index {ARGDICT['outdir']}/{bam_prep}_{ds}.bam", shell=True)
    subprocess.call(f"rm {ARGDICT['outdir']}/*_temp.bam && rm {ARGDICT['outdir']}/*_merged.bam", shell=True)
